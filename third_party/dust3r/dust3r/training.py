...
from occany.must3r_inference import loss_of_one_batch_occany
...
def load_model(args, device):
    ...
    if args.pretrained and not args.resume:
        ...
        if args.multiview:
            # model.load_from_dust3r_checkpoint(args.pretrained)
            model.from_pretrained(args.pretrained)
        else:
            model.from_pretrained(args.pretrained)
            # if args.eval_only:
            #     model.add_forecast_head()
    ...
    #     # Configure compilation for distributed training with dynamic shapes
    #     import torch._dynamo as dynamo
    #     dynamo.config.suppress_errors = True  # Don't fail on compilation errors
        
    #     # Using only backend options, no mode parameter
    #     model = torch.compile(
    #         model, 
    #         backend="inductor",  # Default backend with good performance
    #         fullgraph=False,  # Set to False for better compatibility with DDP
    #         dynamic=True  # Better handling for dynamic shapes
    #     )
    #     print("Model compiled with torch.compile() with dynamic shape optimizations")
...
def train(args):
    ...
    if args.resume is None:
        ...
        print(f"Resuming from last checkpoint {last_ckpt_fname}")
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    args, distill_model, log_writer=None):
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        if args.multiview:
            ...
            # else:
            #     precision = "32"
            # batch_result = loss_of_one_batch_multiview_fast3r(batch, model, criterion, device,
            #                             precision=precision)
            batch_result = loss_of_one_batch_occany(batch, model, criterion, device,
        ...
        # if loss_details["l2_pose"] > 100:
        #     print(f"[WARNING] High l2_pose detected: {loss_details['l2_pose']}")
        #     print("Batch details:", batch[1]['dataset'], batch[1]['label'])
            
        # if loss_details["l3_pose"] > 100:
        #     print(f"[WARNING] High l3_pose detected: {loss_details['l3_pose']}")
        #     print("Batch details:", batch[2]['dataset'], batch[2]['label'])
        ...
        if (data_iter_step % max((len(data_loader) // args.num_save_visual), 1) == 0) and misc.is_main_process() :
            ...
            if args.multiview:
                view1 = batch_result['views'][0]
                view2 = batch_result['views'][1]
                pred1 = batch_result['preds'][0]
                pred2 = batch_result['preds'][1]
            else:
...
def test_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                   ...
                   args, distill_model, log_writer=None, prefix='test'):
    ...
    for idx, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        if args.multiview:
            if bool(args.amp):
                ...
                precision = "32"
            # batch_result = loss_of_one_batch_multiview_fast3r(batch, model, criterion,
        ...
        if args.num_save_visual>0 and (idx % max((len(data_loader) // args.num_save_visual), 1) == 0) and misc.is_main_process() : # save visualizations
            ...
            gt_visual = visualize_results(view1, view2, pred1, pred2, save_dir=save_dir, visualize_type='gt')
            pred_visual = visualize_results(view1, view2, pred1, pred2, save_dir=save_dir, visualize_type='pred')
            # semantic_visual = visualize_semantic(view1, view2, pred1, pred2, save_dir=save_dir)
