...
def make_pairs(imgs, scene_graph='complete',
               prefilter=None, symmetrize=True, 
               force_symmetrize=False, stride=2, start=1):
    pairs = []
    if scene_graph == 'complete':  # complete graph
        ...
        try:
            winsize = int(scene_graph.split('-')[1]) # default = 5
        except Exception:
            winsize = 3
        pairsid = set()
        ...
        for i in range(len(imgs)): # NOTE: default from monst3r
        # for i in range(0, len(imgs), start):
            for j in range(start, stride*winsize + start, stride): # NOTE: this is the default from monst3r
            # for j in range(stride, stride*winsize + stride, stride): # NOTE: I modified to start with stride
                idx = (i + j)
                if iscyclic:
        ...
        for i, j in pairsid:
            # print(imgs[i]['instance'], imgs[j]['instance'])
            pairs.append((imgs[i], imgs[j]))
    ...
    print(f"pairsid: {pairsid}")
    # This condition is true for all the scene graphs except for 'oneref' and 'swin-1'
