...
from occany.utils.image_util import visualize_depth
...
class BasePCOptimizer (nn.Module):
    ...
    def _init_from_views(self, view1, view2, pred1, pred2,
        ...
        self.semantic_2ds = None
        if "semantic_2d" in view1 and "semantic_2d" in view2:
            semantic_2ds = [torch.zeros(hw) for hw in self.imshapes]
            for v in range(len(self.edges)):
                idx = view1['idx'][v]

                semantic_2ds[idx] = view1['semantic_2d'][v]
                idx = view2['idx'][v]
                semantic_2ds[idx] = view2['semantic_2d'][v]
            self.semantic_2ds = semantic_2ds
    ...
    def get_pw_poses(self, scale=True):  # cam to world
    ...
    def save_pts3d_with_colors_and_confs(self, path, conf_thr=0.0):
        pts3d = self.get_pts3d()
        pts3d = [p.detach().cpu().numpy() for p in pts3d]
        rgbs = self.imgs
        confs = self.get_conf()
        confs = [c.detach().cpu().numpy() for c in confs]
        all_data = []
        for i in range(len(pts3d)):
            mask = confs[i] > conf_thr
            filtered_pts3d = pts3d[i][mask]
            filtered_rgbs = rgbs[i][mask] * 255
            filtered_confs = confs[i][mask][:, None]
            data = np.hstack((filtered_pts3d, filtered_rgbs, filtered_confs))
            all_data.append(data)
        all_data = np.vstack(all_data)
        np.savetxt(path, all_data, fmt='%.6f', delimiter=' ', header='x y z r g b conf', comments='')
        print(f"Saved pts3d with colors and confs to {path}")
        return pts3d, rgbs
    ...
    def save_conf_maps(self, path):
        ...
        for i, c in enumerate(conf):
            ...
            conf_img = rgb(c.detach().cpu().numpy())
            cv2.imwrite(f'{path}/conf_{i}.png', conf_img*255)
            # plt.imsave(f'{path}/conf_{i}.png', conf_img, cmap='magma')
    ...
    def save_init_conf_maps(self, path):
        ...
        for i, c in enumerate(conf):
            ...
            conf_img = rgb(c.detach().cpu().numpy())
            cv2.imwrite(f'{path}/init_conf_{i}.png', conf_img*255)
        return conf
    ...
    def save_depth_maps(self, path):
        ...
        # rescale depth maps to [0, 1]
        max_depth = max([d.max() for d in depth_maps]).item()
        ...
        for i, depth_map in enumerate(depth_maps):
            ...
            visualize_depth(depth_map.detach().cpu().numpy(), save_path=img_path, min_val=0.1, max_val=max_depth)

            # cv2.imwrite(img_path, depth_map_colored)
            images.append(Image.open(img_path))
            np.save(f'{path}/depth_frame_{(i):04d}.npy', depth_map.detach().cpu().numpy())

        images[0].save(f'{path}/_depth_maps.gif', save_all=True, append_images=images[1:], duration=100, loop=0)

        return depth_maps

    def save_semantic_2ds(self, path, color_map=None):
        semantic_2ds = self.get_semantic_2ds()
        if semantic_2ds is None:
            return None

        for i, semantic_2d in enumerate(semantic_2ds):
            semantic_2d = semantic_2d.numpy().astype(np.uint8)
            cv2.imwrite(f'{path}/semantic_2d_frame_{(i):04d}.png', semantic_2d)
            if color_map is not None:
                semantic_2d_colored = color_map[semantic_2d]
                semantic_2d_colored = cv2.cvtColor(semantic_2d_colored, cv2.COLOR_RGBA2BGR)
                cv2.imwrite(f'{path}/semantic_2d_colored_frame_{(i):04d}.png', semantic_2d_colored)
        return semantic_2ds
    ...
    def compute_global_alignment(self, init=None, save_score_path=None, save_score_only=False, niter_PnP=10, **kw):
        if init is None:
            ...
            init_fun.init_minimum_spanning_tree(self, save_score_path=save_score_path,
                                                save_score_only=save_score_only, niter_PnP=niter_PnP)
