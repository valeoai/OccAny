...
def minimum_spanning_tree(imshapes, edges, pred_i, pred_j, conf_i, conf_j, im_conf, min_conf_thr,
    ...
    if has_im_poses:
        ...
        # im_focals[i] = estimate_focal(pred_i[i_j], conf_i[i_j])
    ...
    if has_im_poses:
        ...
        for i, j in edges_from_best_to_worse.tolist():
            if im_focals[i] is None:
                # im_focals[i] = estimate_focal(pred_i[edge_str(i, j)], conf_i[edge_str(i, j)])
                im_focals[i] = estimate_focal(pred_i[edge_str(i, j)])

        for i in range(n_imgs):
            if im_poses[i] is None:
                msk = im_conf[i] > min_conf_thr
                print(f'min conf: {min_conf_thr} - {msk.float().mean().item() * 100:.2f}% of points are used for PnP')

                # num_points = int(im_conf[i].numel() * 0.2)
                # if num_points > 0:
                #     conf_values, indices = torch.topk(im_conf[i].view(-1), num_points)
                #     msk = torch.zeros_like(im_conf[i], dtype=torch.bool)
                #     msk.view(-1)[indices] = True
                # else:
                #     msk = torch.zeros_like(im_conf[i], dtype=torch.bool)
...
# def estimate_focal(pts3d_i, conf_i=None, pp=None):
def estimate_focal(pts3d_i, pp=None):
    ...
    #     focal = estimate_focal_knowing_depth(pts3d_i.unsqueeze(0), pp.unsqueeze(0), conf_i.unsqueeze(0), focal_mode='weiszfeld').ravel()
    # else:
