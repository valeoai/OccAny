...
class BaseStereoViewDataset (EasyDataset):
    ...
    def __init__(self, *,  # only keyword arguments
        ...
        self.seed = seed
        self.z_far = z_far
        self.is_metric_scale = False # default is False from dust3r
    ...
    def _crop_resize_if_necessary(self, image, depthmap, intrinsics, resolution, rng=None, info=None):
        ...
        # assert H < W, f"image must be landscape"
        if H > 1.1*W:
    ...
    def _crop_resize_if_necessary_semantic(self, image, depthmap, semantic, intrinsics, resolution, rng=None, info=None):
        """ This function:
            - first downsizes the image with LANCZOS inteprolation,
              which is better than bilinear interpolation in
        """
        ...
        r, b = cx + min_margin_x, cy + min_margin_y
        crop_bbox = (l, t, r, b)
        image, depthmap, semantic, intrinsics = cropping.crop_image_depthmap_semantic(image, depthmap, semantic, intrinsics, crop_bbox)

        # transpose the resolution if necessary
        W, H = image.size  # new size
        assert resolution[0] >= resolution[1]
        assert H < W, "image must be landscape"
        if H > 1.1*W:
            # image is portrait mode
            resolution = resolution[::-1]
            print(f"WARNING: image is portrait in view={info}")
        ...
        if self.aug_focal:
            crop_scale = self.aug_focal + (1.0 - self.aug_focal) * np.random.beta(0.5, 0.5) # beta distribution, bi-modal
            image, depthmap, semantic, intrinsics = cropping.center_crop_image_depthmap_semantic(image, depthmap, semantic, intrinsics, crop_scale)

        if self.aug_crop > 1:
            target_resolution += rng.integers(0, self.aug_crop)
        image, depthmap, semantic, intrinsics = cropping.rescale_image_depthmap_semantic(image, depthmap, semantic, intrinsics, target_resolution) # slightly scale the image a bit larger than the target resolution

        # actual cropping (if necessary) with bilinear interpolation
        intrinsics2 = cropping.camera_matrix_of_crop(intrinsics, image.size, resolution, offset_factor=0.5)
        crop_bbox = cropping.bbox_from_intrinsics_in_out(intrinsics, intrinsics2, resolution)
        image, depthmap, semantic, intrinsics2 = cropping.crop_image_depthmap_semantic(image, depthmap, semantic, intrinsics, crop_bbox)

        return image, depthmap, semantic, intrinsics2
