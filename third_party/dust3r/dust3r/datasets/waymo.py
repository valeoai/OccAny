...
class Waymo (BaseStereoViewDataset):
    ...
    def _get_views(self, pair_idx, resolution, rng):
        ...
        for view_index in [img1, img2]:
            ...
            views.append(dict(
                img=image, # <PIL.Image.Image image mode=RGB size=512x512 at 0x14C352F60B60>
                depthmap=depthmap, # 512x512
                camera_pose=camera_pose,  # 4x4 cam2world
                camera_intrinsics=intrinsics, # 3x3
                dataset='Waymo',
...
if __name__ == '__main__':
    ...
    waymo_root = os.path.join(SCRATCH, "data/waymo_processed")
    
    dataset = Waymo(split='train', ROOT=waymo_root, resolution=512, aug_crop=16)
    idxs = np.arange(0, len(dataset)-1, (len(dataset)-1)//10).astype(np.int32)
    for idx in idxs:
        views = dataset[idx]
        assert len(views) == 2
        ...
        for view_idx in [0, 1]:
            ...
            viz.add_camera(pose_c2w=views[view_idx]['camera_pose'],
                           ...
                           image=colors,
                           cam_size=cam_size)
        os.makedirs('./demo_tmp/waymo', exist_ok=True)
        path = f"./demo_tmp/waymo/waymo_scene_{idx}.glb"
        viz.save_glb(path)
