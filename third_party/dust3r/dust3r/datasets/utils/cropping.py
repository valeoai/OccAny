...
def rescale_image_depthmap_semantic(image, depthmap, semantic, camera_intrinsics, output_resolution, force=True):
    """ Jointly rescale a (image, depthmap, semantic)
        so that (out_width, out_height) >= output_res
    """
    image = ImageList(image)
    input_resolution = np.array(image.size)  # (W,H)
    ...
    scale_final = max(output_resolution / image.size) + 1e-8
    if scale_final >= 1 and not force:  # image is already smaller than what is asked
        return (image.to_pil(), depthmap, semantic, camera_intrinsics)
    output_resolution = np.floor(input_resolution * scale_final).astype(int)
    ...
    if semantic is not None:
        semantic = cv2.resize(semantic, output_resolution, fx=scale_final,
                              fy=scale_final, interpolation=cv2.INTER_NEAREST)
...
def center_crop_image_depthmap_semantic(image, depthmap, semantic, camera_intrinsics, crop_scale):
    """
    Jointly center-crop an image and its depthmap, and adjust the camera intrinsics accordingly.
    ...
    - image: PIL.Image or similar, the input image.
    - depthmap: np.ndarray, the corresponding depth map.
    - semantic: np.ndarray, the corresponding semantic map.
    - camera_intrinsics: np.ndarray, the 3x3 camera intrinsics matrix.
    - crop_scale: float between 0 and 1, the fraction of the image to keep.
    ...
    - cropped_image: PIL.Image, the center-cropped image.
    - cropped_depthmap: np.ndarray, the center-cropped depth map.
    - cropped_semantic: np.ndarray, the center-cropped semantic map.
    - adjusted_intrinsics: np.ndarray, the adjusted camera intrinsics matrix.
    """
    # Ensure crop_scale is valid
    assert 0 < crop_scale <= 1, "crop_scale must be between 0 and 1"
    ...
    if depthmap is not None:
        ...
        assert depthmap.shape[:2] == tuple(image.size[::-1]), "Depthmap size must match image size"
    if semantic is not None:
        # Ensure semantic matches the image size
        assert semantic.shape[:2] == tuple(image.size[::-1]), "Semantic size must match image size"
    ...
    return image.to_pil(), depthmap, semantic, adjusted_intrinsics
...
def crop_image_depthmap_semantic(image, depthmap, semantic, camera_intrinsics, crop_bbox):
    """
    Return a crop of the input view.
    """
    image = ImageList(image)
