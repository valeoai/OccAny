...
def save_semantic(semantic, path, class_mapping, image=None):
    # semantic_masks = self.semantic_masks
    # Save semantic pseudolabel
    semantic_2d_pseudolabel_colors = class_mapping.get_color(semantic)  # [H, W, 4]
    # Convert to uint8 before color conversion
    semantic_2d_pseudolabel_colors = semantic_2d_pseudolabel_colors.astype(np.uint8)
    # Convert RGBA to BGRA for OpenCV compatibility
    semantic_2d_pseudolabel_colors = cv2.cvtColor(semantic_2d_pseudolabel_colors, cv2.COLOR_RGBA2BGRA)
    
    if image is not None:
        # Create semantic overlay
        semantic_overlay = cv2.addWeighted(image, 0.5, semantic_2d_pseudolabel_colors[:, :, :3], 0.5, 0)
        cv2.imwrite(path, semantic_overlay)
    else:
        cv2.imwrite(path, semantic_2d_pseudolabel_colors[:, :, :3])
    print(f"Saved semantic {path}")
