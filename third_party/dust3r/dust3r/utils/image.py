...
# GroundingDinoImgNorm = tvf.Compose([tvf.ToTensor(), tvf.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))])
...
def crop_img(img, size, square_ok=False, nearest=False, crop=True):
    W1, H1 = img.size
    if type(size) == tuple and size[0] == 224:
        # resize short side to 224 (then crop)
        # img = _resize_pil_image(img, round(size * max(W1/H1, H1/W1)), nearest=nearest)
        # new_size = math.ceil(80 * max(W1/H1, H1/W1))
        # img = _resize_pil_image(img, new_size, nearest=nearest)

        img = img.resize((size[0], size[1]), PIL.Image.LANCZOS)
    else:
...
def load_images(folder_or_list, size, square_ok=False, 
                verbose=True, dynamic_mask_root=None, crop=True, fps=0, num_frames=110):
    ...
    for path in folder_content:
        ...
        if path.lower().endswith(supported_images_extensions):
            ...
            gdino_img, _ = GroundingDinoImgNorm(img, None)
            single_dict = dict(
                img=ImgNorm(img)[None],
                img_grounding_dino=gdino_img[None],
