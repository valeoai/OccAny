...
def normalize_pointcloud_forecast(pts1, pts2, pts_forecast, norm_mode='avg_dis', valid1=None, valid2=None, valid_forecast=None, ret_factor=False):
    """ renorm pointmaps pts1, pts2, pts_forecast with norm_mode
    """
    assert pts1.ndim >= 3 and pts1.shape[-1] == 3
    assert pts2 is None or (pts2.ndim >= 3 and pts2.shape[-1] == 3)
    assert pts_forecast is None or (pts_forecast.ndim >= 3 and pts_forecast.shape[-1] == 3)
    ...
    if norm_mode == 'avg':
        ...
        nan_pts_forecast, nnz_forecast = invalid_to_zeros(pts_forecast, valid_forecast, ndim=3) if pts_forecast is not None else (None, 0)
        all_pts = torch.cat([p for p in (nan_pts1, nan_pts2, nan_pts_forecast) if p is not None], dim=1)
        ...
        if dis_mode == 'dis':
            ...
            if pts2 is not None:
                ...
                pts2 = pts2 * warp_factor[:, W1 * H1:W1 * H1 + W2 * H2].view(-1, H2, W2, 1)
            if pts_forecast is not None:
                H3, W3 = pts_forecast.shape[1:-1]
                pts_forecast = pts_forecast * warp_factor[:, W1 * H1 + W2 * H2:].view(-1, H3, W3, 1)
            all_dis = log_dis  # this is their true distance afterwards
        else:
            raise ValueError(f'bad {dis_mode=}')

        norm_factor = all_dis.sum(dim=1) / (nnz1 + nnz2 + nnz_forecast + 1e-8)
    else:
        # gather all points together (joint normalization)
        nan_pts1 = invalid_to_nans(pts1, valid1, ndim=3)
        nan_pts2 = invalid_to_nans(pts2, valid2, ndim=3) if pts2 is not None else None
        nan_pts_forecast = invalid_to_nans(pts_forecast, valid_forecast, ndim=3) if pts_forecast is not None else None
        all_pts = torch.cat([p for p in (nan_pts1, nan_pts2, nan_pts_forecast) if p is not None], dim=1)
    ...
    if pts_forecast is not None:
        res = res + (pts_forecast / norm_factor,)
    if ret_factor:
        res = res + (norm_factor,)
    return res
...
@torch.no_grad()
def get_joint_pointcloud_depth_multiview(zs, valid_masks, quantile=0.5):
    # set invalid points to NaN
    _zs = [invalid_to_nans(z, valid_mask).reshape(len(z), -1) for z, valid_mask in zip(zs, valid_masks)]
...
@torch.no_grad()
def get_joint_pointcloud_center_scale_multiview(list_pts, list_valid_masks, z_only=False, center=True):
    # set invalid points to NaN
    _pts = [invalid_to_nans(pts, valid_mask).reshape(len(pts), -1, 3) for pts, valid_mask in zip(list_pts, list_valid_masks)]

    # compute median center
    _center = torch.nanmedian(torch.cat(_pts, dim=1), dim=1, keepdim=True).values  # (B,1,3)
    if z_only:
        _center[..., :2] = 0  # do not center X and Y

    # compute median norm
    _norm = (torch.cat(_pts, dim=1) - _center if center else torch.cat(_pts, dim=1)).norm(dim=-1)
