...
class DepthAnything3Net(nn.Module):
    ...
    def _process_ray_pose_estimation(
        ...
        if "ray" in output and "ray_conf" in output:
            pred_extrinsic, pred_focal_lengths, pred_principal_points = get_extrinsic_from_camray(
                output.ray,
                output.ray_conf.clone(),  # Clone to prevent in-place modification in compute_optimal_rotation_intrinsics_batch
                output.ray.shape[-3],
                output.ray.shape[-2],
            )
    ...
    def _extract_auxiliary_features(
        ...
        for feat_tuple, feat_layer in zip(feats, feat_layers):
            # feat_tuple is (processed_feat, raw_state) - use processed_feat for aux features
            feat = feat_tuple[0] if isinstance(feat_tuple, tuple) else feat_tuple
