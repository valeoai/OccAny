...
class Sam3Processor:
    ...
    def set_image(self, image, state=None):
        ...
        # backbone_out is the dict returned by SAM3VLBackbone.forward_image(image):
        # - "backbone_fpn": list of multi-scale feature maps from the vision backbone;
        #   for a 1008x1008 input image the 3 levels have shapes
        #   [1, 256, 288, 288], [1, 256, 144, 144], [1, 256, 72, 72]
        # - "vision_pos_enc": list of positional encodings matching backbone_fpn,
        #   with the same shapes as the corresponding feature maps
        # - "vision_features": final (last-level) visual feature map used by the transformer;
        #   identical to backbone_fpn[2] with shape [1, 256, 72, 72]
        # - "sam2_backbone_out": optional SAM2-style backbone outputs with the same keys
