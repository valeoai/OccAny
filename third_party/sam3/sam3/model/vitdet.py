...
class Attention(nn.Module):
    ...
    def _apply_rope(self, q, k, seq_len: Optional[int] = None) -> Tuple[Tensor, Tensor]:
        if not self.use_rope:
            return q, k

        # Check if we need to recompute freqs_cis for different input size
        if seq_len is not None and self.freqs_cis is not None:
            expected_seq_len = self.freqs_cis.shape[0]
            if self.cls_token:
                expected_seq_len -= 1  # Account for cls token
            
            if seq_len != expected_seq_len:
                # Compute freqs_cis dynamically using NTK-aware interpolation
                # This method preserves high-frequency information better than simple scaling
                H = W = int(math.sqrt(seq_len))
                original_size = int(math.sqrt(expected_seq_len))
                
                # NTK-aware interpolation: scale theta based on resolution ratio
                # This preserves the relative position encoding quality
                scale_ratio = original_size / H
                
                # Use scaled theta for better preservation of position information
                # When downscaling (H < original), increase theta to maintain resolution
                # When upscaling (H > original), decrease theta 
                theta_scaled = self.rope_theta * (scale_ratio ** 2)
                
                
                # apply position interpolation for smooth transitions
                # scale_pos = scale_ratio if self.rope_interp else 1.0
                scale_pos = 1.0
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        q, k = self._apply_rope(q, k, seq_len=L)
        if self.use_rel_pos:
            q, k = concat_rel_pos(
                q.flatten(0, 1),
                k.flatten(0, 1),
