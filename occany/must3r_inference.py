...
def inference_depth_completion(img_views, gen_views,
    ...
    if sam_feats is not None:
        ...
        for i in range(len(sam_feats)):
            sam_feat_var = F.interpolate(sam_feats[i].reshape(B*nimgs, -1, sam_feats[i].shape[3], sam_feats[i].shape[4]), (H, W), mode="bilinear", align_corners=False)
    ...
    pts_features = [pts3d, rgb, conf.unsqueeze(-1) - 1.0, *sam_feats_resized]
    
    # [3, 3, 1, 256, 64, 32]
    if sam_model == "SAM2":
        assert [t.shape[-1] for t in pts_features] == [3, 3, 1, 256, 64, 32], "pts_features shape is not correct for SAM2"
    elif sam_model == "SAM3":
        assert [t.shape[-1] for t in pts_features] == [3, 3, 1, 256, 256, 256], "pts_features shape is not correct for SAM3"
 
    
    pts_features = torch.cat(pts_features, dim=-1)
    ...
    if gen_views is not None and len(gen_views) > 0:        
        ...
        raymap_batch_size = min(12, nraymaps)  # Process up to 12 raymaps at a time
        # cond_features_list = []
        raymap_outs = []
        sam_feats_raymaps = []
        for raymap_start in range(0, nraymaps, raymap_batch_size):
            raymap_end = min(raymap_start + raymap_batch_size, nraymaps)
            ...
            novel_view_rgbs_batch = novel_view_rgbs[:, raymap_start:raymap_end]
            
            # Create conditioning for this batch of raymaps
            projected_pts3d_batch = create_gen_conditioning(
                pts3d, pts_features, focal, raymap_c2w_batch, 
                raymap_views=gen_views_batch,
                return_projected_pts3d=True)
            breakpoint()
            # cond_features_list.append(cond_features_batch)
            
            # Save the batch slice for later use
            # raymap_c2w_batch_saved = raymap_c2w_batch.clone()
            
            # Clear intermediate tensors to free memory
            # del raymap_c2w_batch, cond_features_batch
            # torch.cuda.empty_cache()
            # del raymap_c2w_batch
        
            # # Concatenate all raymap batch results along dimension 1
            # cond_features = torch.cat(cond_features_list, dim=1)
            # del cond_features_list
            # # torch.cuda.empty_cache()
            ...
            with torch.autocast("cuda", dtype=dtype):
                ...
                raymap_out, pose_raymap_out, sam_feats_raymap = inference_render(
                    decoder=decoder_gen,
                    x=x_ray,
                    pos=pos_ray,
                ...
                sam_feats_raymaps.append(sam_feats_raymap)

            with torch.autocast("cuda", dtype=torch.float32):
                # raymap_out = postprocess(raymap_out, pose_raymap_out, 
                #                 pointmaps_activation=pointmaps_activation,
                #                 compute_cam=True)
                raymap_out = postprocess(raymap_out, 
                                pose_out=raymap_c2w_batch, 
                                pointmaps_activation=pointmaps_activation,
                                compute_cam=True)
                # if use_local_points_with_pose_as_pts3d:
                #     raymap_out['pts3d'] = raymap_out['pts3d_from_local_and_pose']
                raymap_out["c2w_input"] = raymap_c2w_batch
                raymap_outs.append(raymap_out)
        
        # Concatenate raymap batch results
        if len(raymap_outs) > 0:
            if sam_feats_raymaps[0] is not None:
                sam_feats_raymaps_cat = [[], [], []]
                for j in range(3):
                    sam_feats_raymaps_cat[j] = torch.cat([s[j] for s in sam_feats_raymaps], dim=1)
                sam_feats_raymap = sam_feats_raymaps_cat
            else:
                sam_feats_raymap = None
            # Concatenate raymap_out dictionaries
            raymap_out = raymap_outs[0]
            for raymap_out_i in raymap_outs[1:]:
                raymap_out = concat_preds(raymap_out, raymap_out_i)
            
    
    return img_out, raymap_out, x_ray, recon_2_gen_mapping
